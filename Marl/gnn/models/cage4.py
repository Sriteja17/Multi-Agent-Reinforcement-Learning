...
from .memory_buffer import MultiPPOMemory
from .utils import combine_marl_states
...
def extract_hosts(x, servers, n_servers, users, n_users):
    ...
    servers = servers.to(x.device)
    users = users.to(x.device)
...
class InductiveActorNetwork(nn.Module):
    ...
    def forward(self, x, ei, global_vec, servers, n_servers, users, n_users, action_edges, multi_subnet):
        ...
        x = x.to(DEVICE)
        ei = ei.to(DEVICE)
        global_vec = global_vec.to(DEVICE)

        servers = servers.to(DEVICE)
        n_servers = n_servers.to(DEVICE)

        users = users.to(DEVICE)
        n_users = n_users.to(DEVICE)

        action_edges = action_edges.to(DEVICE)



        action_edges = action_edges.to(x.device)
        rtrs = action_edges.unique(sorted=True).squeeze(-1)
        bs = rtrs.size(0) // 9
        rtrs = rtrs.reshape(bs, 9)
        rtrs = rtrs.to(x.device)
        if multi_subnet:
...
class InductiveGraphPPOAgent():
    ...
    def __init__(self, in_dim, gamma=0.99, lmbda=0.95, clip=0.1, bs=5, epochs=6,
        ...
        self.actor = InductiveActorNetwork(
            in_dim,
            concat_edges=concat_edges,
            **a_kwargs
        ).to(DEVICE)

        self.critic = InductiveCriticNetwork(
            in_dim,
            **c_kwargs
        ).to(DEVICE)

        print("Actor :", next(self.actor.parameters()).device)
        print("Critic:", next(self.critic.parameters()).device)
    ...
    def learn(self, verbose=False):
        ...
        for e in range(self.epochs):
            ...
            for b_idx,b in enumerate(batches):
                ...
                # Forward pass 
                print("Batch device:", batched_states[0].device)
...
def load(in_f):
    ...
    data = torch.load(in_f,map_location=DEVICE)
