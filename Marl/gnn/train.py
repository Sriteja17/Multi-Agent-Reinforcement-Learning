...
HYPER_PARAMS = SimpleNamespace(
    N = 5,             # How many episodes before training
    workers = 5,       #(was 25) How many envs can run in parallel
    bs = 512,          # How many steps to learn from at a time
    episode_len = 500,
    training_episodes = 10000, # Realistically, stops improving around 50k
    epochs = 4
)

N_AGENTS = 5 
MAX_THREADS = 20 # (was 20)5 per subnet (20 for agent 4, 4 for all others)
...
def train(agents, hp, seed=SEED):
    ...
    for e in range(hp.training_episodes // hp.N):
        ...
        # last_losses = []
        # for i in range(N_AGENTS):
        #     last_losses.append(learn(i))
        
        # sequential because my pc crying :()
        ...
        torch.save(log, f'evaluation/gnn/{hp.fnames}.pt')

        # Checkpoint model states 
        for i in range(N_AGENTS):
            agent = agents[i]
            agent.save(outf=f'checkpoints/gnn/{hp.fnames}-{i}_checkpoint.pt')

            if e % 10_000 < hp.N and e > hp.N:
                agent.save(outf=f'checkpoints/gnn/{hp.fnames}-{i}_{e//1000}k.pt')
...
if __name__ == '__main__':
    ...
    if not os.path.exists('evaluation/gnn'):
        os.mkdir('evaluation/gnn')

    # Add directory for model weights 
    if not os.path.exists('checkpoints/gnn'):
        os.mkdir('checkpoints/gnn')
